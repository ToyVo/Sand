...
import { random, fastItoXYBordering } from "./util.js";
 ...
 */
export const elements = new Uint32Array([
  BACKGROUND,
  WALL,
  SAND,
  WATER,
...
function ACID_ACTION(x, y, i) {
  ...
  if (random() < 10) {
    ...
    for (yLocsIter = 0; yLocsIter !== 3; yLocsIter++) {
      ...
      if (random() < 25 && yIter !== down) continue;
...
function BURNING_THERMITE_ACTION(x, y, i) {
  ...
  if (random() < 8) {
    ...
    if (adjWall !== -1) gameImagedata32[adjWall] = BACKGROUND;

    const belowWall = below(y, i, WALL);
    if (belowWall !== -1) gameImagedata32[belowWall] = BACKGROUND;
  }
  ...
  if (belowFire !== -1) gameImagedata32[belowFire] = BACKGROUND;
  if (doGravity(x, y, i, false, 99)) return;
